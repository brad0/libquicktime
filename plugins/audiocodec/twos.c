...
static int decode(quicktime_t *file, 
  ...
  if(!codec->decode_initialized)
    ...
    if(bits <= 16)
      {
      codec->decode_sample_buffer_i    = malloc(sizeof(*(codec->decode_sample_buffer_i)) *
                                                file->atracks[track].channels);
      codec->decode_sample_buffer_i[0] = malloc(sizeof(*(codec->decode_sample_buffer_i[0])) *
                                                file->atracks[track].channels *
                                                SAMPLES_PER_BLOCK);
      ...
      {
      codec->decode_sample_buffer_f    = malloc(sizeof(*(codec->decode_sample_buffer_f)) *
                                                file->atracks[track].channels);
      codec->decode_sample_buffer_f[0] = malloc(sizeof(*(codec->decode_sample_buffer_f[0])) *
                                                file->atracks[track].channels *
                                                SAMPLES_PER_BLOCK);
      ...
      switch(bits)
        ...
        case 16:
          for(i = 0; i < codec->decode_sample_buffer_size; i++)
            ...
            for(j = 0; j < track_map->channels; j++)
              {
              codec->decode_sample_buffer_i[j][i] =
                ((int16_t)codec->decode_buffer_ptr[0]) << 8 |
        ...
        case 24:
          for(i = 0; i < codec->decode_sample_buffer_size; i++)
            ...
            for(j = 0; j < track_map->channels; j++)
              {
              codec->decode_sample_buffer_i[j][i] =
                (float)((((int)codec->decode_buffer_ptr[0]) << 16) | 
