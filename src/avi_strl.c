...
/* length of track */
				samples = quicktime_read_int32_le(file);
                                fprintf(stderr, "*** Time: %f\n", (float)samples/(float)bytes_per_second);
...
/* Should be enough entries allocated in quicktime_stsc_init_table */
			...
			trak->mdia.minf.stbl.stsc.total_entries = 1;
                ...
                {
                fprintf(stderr, "Samples: %d, sample_rate: %d bytes_per_second: %d\n",
                        samples, sample_rate, bytes_per_second);
			trak->mdia.minf.stbl.stsc.table[0].samples =
                          (int)(((float)(samples)*(float)(sample_rate))/(float)bytes_per_second);
			trak->mdia.minf.stbl.stsc.total_entries = 1;
                }
#endif
	}
